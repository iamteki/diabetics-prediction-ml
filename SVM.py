...
# Train the SVM classifier using GridSearchCV for hyperparameter tuning
svm_classifier = GridSearchCV(SVC(probability=True, random_state=42), param_grid, cv=5, refit=True, verbose=2)
...
def predict_diabetes_svm(input_data):
    standardized_input = scaler.transform([input_data])
    probability = svm_classifier.predict_proba(standardized_input)[0][1]  # Probability of having GDM
...
def predict_from_excel(input_file, output_file):
    ...
    # Make predictions
    predictions = svm_classifier.predict(input_features)
    probabilities = svm_classifier.predict_proba(input_features)[:, 1]
